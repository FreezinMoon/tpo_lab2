package test.java.integration;

import main.java.modules.log.LnModule;
import main.java.modules.log.Log2Module;
import main.java.modules.log.Log3Module;
import main.java.modules.log.Log5Module;
import main.java.modules.trig.*;
import main.java.modules.trig.real.CosModuleReal;
import main.java.modules.trig.real.SecModuleReal;
import main.java.modules.trig.real.SinModuleReal;
import main.java.modules.trig.real.TanModuleReal;
import main.java.system.PiecewiseFunction;
import org.junit.Before;
import org.junit.Test;

import static org.junit.Assert.assertFalse;
import static org.mockito.Mockito.*;

public class Step2SecTanTest {

    private CotModule cotMock;

    private Log2Module log2Mock;
    private Log3Module log3Mock;
    private Log5Module log5Mock;
    private LnModule lnMock;

    private PiecewiseFunction systemUnderTest;

    @Before
    public void setUp() {
        SinModule sinReal = new SinModuleReal(1e-3);
        CosModule cosReal = new CosModuleReal(sinReal);
        TanModule tanReal = new TanModuleReal(sinReal, cosReal);
        SecModule secReal = new SecModuleReal(cosReal);

        cotMock = mock(CotModule.class);

        log2Mock = mock(Log2Module.class);
        log3Mock = mock(Log3Module.class);
        log5Mock = mock(Log5Module.class);
        lnMock = mock(LnModule.class);

        systemUnderTest = new PiecewiseFunction(tanReal, cotMock, secReal, log2Mock, log3Mock, log5Mock, lnMock);
    }

    @Test
    public void testTrigPartSecAndTanReal() {
        double x = -Math.PI / 4;

        when(cotMock.cot(x)).thenReturn(-1.0);

        double result = systemUnderTest.calculate(x);
        System.out.println("Step2SecTan -> f(" + x + ") = " + result);

        verify(cotMock).cot(x);

        assertFalse(Double.isNaN(result));
    }

    @Test
    public void testPositiveXMocksLog() {
        double x = 2.0;
        when(log2Mock.log2(2.0)).thenReturn(1.0);
        when(log3Mock.log3(2.0)).thenReturn(0.63);
        when(log5Mock.log5(2.0)).thenReturn(0.43);
        when(lnMock.ln(2.0)).thenReturn(0.693);

        double result = systemUnderTest.calculate(x);
        System.out.println("Step2SecTan -> f(2.0) = " + result);

        verifyNoInteractions(cotMock);

        assertFalse(Double.isNaN(result));
    }
}
